...
Ported from PixelRAG's answer.py — logic unchanged
(rerank -> optional figure crop -> synthesise). Kept for future Q&A reuse per
the brief; nothing in the Requirements/Design modules calls this yet, but
requirements_extractor.py and alignment_checker.py are modeled on the same
rerank/crop/synthesise vision pattern this module established, sharing its
image_to_data_url helper via common.py instead of duplicating it.
"""
...
from ai.ingest.search import SearchResult  # noqa: E402
from config import RERANKER, RERANK_TOP_N, CROP_MIN_PX, ANSWER_MODEL  # noqa: E402
...
def _rerank(query: str, results: list[SearchResult], client, model: str) -> list[SearchResult]:
    """
    Route to the reranker configured in config.yaml via reranker: gpt-4o|jina-reranker-v2-base-multimodal.
    ...
    try:
        resp = client.chat.completions.create(
            ...
            messages=[{
                "role": "user",
                "content": [
                    image_to_data_url(img, detail="high"),
...
def synthesise_answer(
    ...
    if crop is not None:
        ...
        content.append(image_to_data_url(crop, detail="high"))
        content.append({
            "type": "text",
            "text": f"[Full page — {top_pages[0].source}, page {top_pages[0].page}]"
        })
        content.append(image_to_data_url(top_pages[0].image, detail="high"))
        for i, r in enumerate(top_pages[1:]):
            img = r.image
            if img is None:
                continue
            content.append({"type": "text", "text": f"[Image {i + 2}: {r.source}, page {r.page}]"})
            content.append(image_to_data_url(img, detail="high"))
    else:
